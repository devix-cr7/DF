...
- **Language**: every piece of UI text — shell, dashboard, all 36 tools' titles/descriptions, and every button/label/placeholder inside each tool — is translated across all 7 languages, resolved through `useT()` (`src/hooks/useT.ts`) and the dictionaries in `src/i18n/translations.ts`. Code/technical content (curl commands, JSON, SQL, hex colors, table/field names) is intentionally left untranslated and forced left-to-right even on the Arabic layout, since code isn't meant to be localized.
