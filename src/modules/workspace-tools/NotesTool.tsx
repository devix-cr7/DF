...
export default function NotesTool() {
  ...
  function create() {
    const note: Note = { id: crypto.randomUUID(), title: t("notes.untitled"), body: "", updatedAt: Date.now() };
    setNotes((n) => [note, ...n]);
    setActiveId(note.id);
  }
      ...
      <div className="grid h-full grid-cols-1 gap-4 md:grid-cols-[220px_1fr]">
        <div className="flex flex-col gap-2">
          ...
          <div className="min-h-[120px] flex-1 space-y-1 overflow-y-auto md:min-h-0">
            ...
            {notes.length === 0 && (
              <p className="px-2 py-6 text-center text-[12px] text-forge-faint">{t("notes.empty_list")}</p>
            )}
          </div>
        </div>
